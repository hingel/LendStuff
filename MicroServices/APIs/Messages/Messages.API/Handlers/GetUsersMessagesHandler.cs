...
public class GetUsersMessagesHandler(IRepository<InternalMessage> repository) : IRequestHandler<GetUsersMessages, ServiceResponse<IEnumerable<MessageDto>>>
{
	public async Task<ServiceResponse<IEnumerable<MessageDto>>> Handle(GetUsersMessages request, CancellationToken cancellationToken)
	{
		...
		var result = await repository
			.FindByKey(m => m.SentToUserId == request.UserId);
