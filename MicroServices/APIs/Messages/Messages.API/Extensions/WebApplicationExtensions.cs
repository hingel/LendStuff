...
public static class WebApplicationExtensions
{
	public static WebApplication MapMessageEndPoints(this WebApplication app)
	{
		...
		app.MapGet("/getUsersMessages", async (IMediator mediator, Guid UserId) =>
			await mediator.Send(new GetUsersMessages(UserId))); //.RequireAuthorization();
		app.MapDelete("/deleteMessage", async (IMediator mediator, Guid MessageId) =>
			await mediator.Send(new DeleteMessageCommand(MessageId))); //.RequireAuthorization();
